...
ggplot(inflation_df, aes(x = observation_date, y = CPILFESL)) +
  ...
  scale_x_date(breaks = seq(from = as.Date("1957-01-01"),
                            ...
                            by = "5 years"),
               date_labels = "%Y") +
  labs(title = "Overtime American Price Increase ($28.50 in 1957)",
       x = "Year", y = "Price ($)") +
  theme_bw()
...
ggplot(filter(unempop_df, row_number() != 1), aes(x = `Labor Force Statistics from the Current Population Survey` , y = ...2)) +
  geom_line(color = "#ba040d", size = 1) +
  scale_x_continuous(breaks = seq(1948, 2025, by = 10)) +
  scale_y_continuous(breaks = seq(0, 20000, by = 3000)) +
  geom_point(size = 1) +
